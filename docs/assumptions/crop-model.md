...
|---|---|---|---|
| `variety` | "Beef tomato (EYRE RZ F1-class, ~230-280g)" (was "Beefsteak tomato", corrected 2026-08-26) | **PLACEHOLDER, researched** | The old label was a pure cosmetic placeholder. Re-decided through three lenses at the user's request: (1) regional agronomic fit — Northern Greece (this project's own weather region) has named beef-type F1 hybrids recommended for it, but the two checked (AMATI F1: semideterminate, conflicts with this project's continuous indeterminate 330-day cycle; BELLADONA F1: indeterminate but explicitly not recommended for autumn/winter planting, conflicting with this project's September start) were both rejected; (2) market demand — beef (~1€/kg, 65-70 kg/m²/yr) vs. cherry (~3€/kg, 20-40 kg/m²/yr) turn out close in gross revenue/m² once real cherry yield and (materially higher) cherry labor cost are factored in, so beef was chosen as the better net fit and the one already matching this project's existing single-stem high-wire density assumption; (3) processing/value-add — Greece's real tomato-processing industry is a separate field-grown supply chain, not a realistic outlet for greenhouse hydroponic production, so no value-add angle changes this decision. Finally cross-checked against a real Northern Greece CHP+hydroponic precedent (Θερμοκήπια Σαββίδη / "Dramello Fresh" brand, 250-280g beef tomato) and matched to a real, Greece-available hybrid in that same size class, **EYRE RZ F1** (Rijk Zwaan) — indeterminate growth habit inferred with high confidence but not explicitly confirmed in any source found. Full research trail: `docs/papers/tomato-variety-selection-northern-greece.md`. **Follow-up completed 2026-08-26**: `target_fruit_weight_g`, `fruits_per_truss`, `stems_per_plant`, `photosynthesis_t_min/opt/max_c`, `fruit_set_t_min/opt/max_c`, `canopy_light_extinction_coeff`, and `maintenance_respiration_fraction_per_day` are now all `CropParams` fields (see rows below) — a different real variety now just means different config values, not a code change (except `stems_per_plant`, which does branch model behavior, by design — see its own row). |
| `target_fruit_weight_g` | 255.0 g | **SOURCED** | Midpoint of EYRE RZ F1's 230-280g spec. Not an input to the crop model's own growth physics (still driven by `fruit_partition_fraction_max`/dry-matter accumulation) — a reporting/derivation constant converting cumulative fruit yield into truss-rate metrics. See `docs/papers/tomato-variety-selection-northern-greece.md`. |
| `fruits_per_truss` | 5.0 | **SOURCED (class-level)** | Generic beef/TOV benchmark (4-6/truss), not an EYRE RZ F1 datasheet figure (not publicly available). Used alongside `target_fruit_weight_g` for the truss-rate derivation. |
| `stems_per_plant` | 1 | **SITE-SPECIFIC** | 1 = single-stem (this project's density/reference-density figures were sourced against single-stem practice), 2 = two-stem (checked during variety selection — Belladona F1). Unlike the other variety fields, this one **does** change model behavior: `twin/crop_model.py`'s `_lai` scales canopy density by `density_plants_per_m2 * stems_per_plant` (a two-stem plant carries roughly twice the leaf area of a single-stem one at the same plants/m²), and the API's truss-rate derivation divides by the same effective stem density, reporting trusses *per stem* (how a real grower actually tracks it), not per plant. |
| `photosynthesis_t_min_c` / `_t_opt_c` / `_t_max_c` | 10.0 / 27.0 / 35.0 | **SOURCED** | Bell-shaped photosynthesis temperature-response thresholds, moved here from `twin/crop_model.py` module constants (`T_MIN_C`/`T_OPT_C`/`T_MAX_C`) 2026-08-26 so a variety's temperature tolerance is a config value, not a code edit. Same values, no behavior change at the EYRE RZ F1-class default. Source: as before (Frontiers 10.3389/fpls.2017.00365; researchgate 323225604 review). |
| `fruit_set_t_min_c` / `_t_opt_c` / `_t_max_c` | 12.0 / 18.0 / 24.0 | **SOURCED** | Much narrower fruit-set temperature tolerance (pollen viability/fruit set is far more temperature-sensitive than photosynthesis). Moved from module constants (`FRUIT_SET_T_*`) for the same reason — and this is **the trait that actually differs meaningfully between real varieties** checked during variety selection (e.g. Merlice F1's "very good fruit set at high temperatures" vs. this EYRE RZ F1-class default). Source: as before, `docs/papers/tomato-fruit-set-temperature-sensitivity.md`. |
| `canopy_light_extinction_coeff` | 0.75 | **SOURCED** | Beer-Lambert canopy light-extinction coefficient (0.7-0.9 reported range for high-wire tomato canopies), varies somewhat with a variety's leaf habit. Moved from a module constant (`CANOPY_LIGHT_EXTINCTION_COEFF`). |
| `maintenance_respiration_fraction_per_day` | 0.015 | **PLACEHOLDER** | Plant vigor (and respiration load) varies somewhat by variety. Moved from a module constant (`MAINTENANCE_RESPIRATION_FRACTION_PER_DAY`), not individually sourced. |
...
| `simulation.duration_days` | 330 (was 150, corrected 2026-08-25) | **SOURCED** | How long the simulated run covers, not a `crop:` field but tightly coupled to the crop cycle. 150 days (~5 months) had no rationale behind it -- caught when the user asked directly. Indeterminate tomato has no natural end-of-life (it keeps growing, flowering and fruiting until disease, environmental stress, or the grower ends the cycle); real commercial greenhouse tomato cropping cycles run **~10-11 months** before replanting. 330 days (11 months) matches that. `twin/params.py`'s dataclass default was also updated to match (was inconsistently 120). The model itself still has no plant-termination mechanism -- a longer `duration_days` just keeps simulating continued growth, consistent with indeterminate biology. See `docs/papers/tomato-crop-cycle-duration.md`. |
...
The previous fix corrected the *direction* for warm nights, but the user immediately caught the
other half: sweeping the full night-setpoint range showed yield still climbing *without limit* as
the setpoint dropped all the way to 5°C (8.137 kg/m² at 5°C > 6.466 kg/m² at the literature-optimal
17°C) — real tomato night-temperature optima are a genuine bell curve (too cold hurts as much as
too warm), not "colder is always better below the warm-side penalty."

Root cause: nothing in the model penalized cold specifically — `_temperature_response`
(T_MIN=10, T_OPT=27, T_MAX=35) is calibrated for *photosynthesis*, which tomato leaves tolerate
over a wide range, so it barely penalizes cold nights near its floor. Fruit set (whether a flower
actually becomes a fruit) is far more temperature-sensitive than photosynthesis — pollen viability
and pollen-tube growth fail in a much narrower window — but nothing in the model captured that
distinct mechanism.

Added `_fruit_set_temp_response` (`FRUIT_SET_T_MIN_C=12, FRUIT_SET_T_OPT_C=18, FRUIT_SET_T_MAX_C=24`
— SOURCED, see `docs/papers/tomato-fruit-set-temperature-sensitivity.md`): below ~55°F (12.8°C)
promotes blossom drop and poor pollen vigor; ideal fruit set sits in a narrow 60-70°F
(15.6-21.1°C) window; above ~75°F (24°C) interferes with pollen-tube growth. This multiplies the
fruit partition fraction directly.

**First attempt didn't work**: gating this on the *current hour's* temperature has no effect at
night at all, because net dry matter is never positive at night (zero light, zero gross
assimilation) — so no fruit credit is ever computed at night regardless of how cold it is; the
penalty only ever saw daytime temperature (a fixed 20°C in the sweep), completely missing the
night-setpoint being swept. Fixed by evaluating `_fruit_set_temp_response` against
`CropState.recent_temp_ema_c`, an exponential moving average of `temp_in_c` (half-life
`FRUIT_SET_TEMP_EMA_HALF_LIFE_HOURS=12h`, PLACEHOLDER engineering choice representing roughly how
long flower/pollen-tube development integrates recent conditions) instead of the instantaneous
