...
`summary.total_recirculation_elec_kwh`, and their sum `summary.total_electricity_kwh`.
...
- **Fertigation pumps (Spagnol BravoJet)** are a real electrical consumer at this
  greenhouse not modeled at all yet — smaller than the three above, and would need
  new modeling (irrigation volume/schedule) rather than reuse of an existing
  physical output. Deferred as a later, lower-priority step.
