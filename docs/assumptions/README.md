...
- **Hydroponic Level A complete (2026-08-26), Levels B/C not started** (`hydroponics.md`) — system type corrected from the wrong "NFT" to "drip_substrate", and EC/pH/drainage-fraction targets now really drive irrigation water volume, drainage volume, fertigation pump electricity, and an approximate fertilizer mass (all derived from the crop's own transpiration, same reporting-layer pattern as the electricity fields above). **Level B (not started)**: the real physiological lever — EC affects fruit fresh weight (higher EC → smaller, more concentrated fruit; lower EC → larger, more diluted fruit) — nothing in `twin/crop_model.py` reads `ec_target_ms_cm` yet, so changing it today only changes cost/water reporting, not simulated yield or fruit size. **Level C (not started, likely low priority)**: substrate moisture dynamics, individual macro/micronutrients (N/P/K/Ca/Mg) rather than aggregate EC, pH as a real availability-threshold mechanism — same "probably not worth the added complexity without a specific sourced mechanism" reasoning as senescence/leaf-abscission above.
