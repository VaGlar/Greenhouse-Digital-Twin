...
- [`economics.md`](./economics.md) — economic layer, phase 1: physical electricity consumption (dehumidification + ventilation + recirculation fans + fertigation pump), not yet priced
- [`hydroponics.md`](./hydroponics.md) — hydroponic system type, EC/pH/drainage targets, fertigation water/electricity/fertilizer-mass derivation (Level A, complete); Levels B/C tracked below
...
- **Economic layer is physical-only so far** (`economics.md`) — dehumidification, ventilation, recirculation-fan, and (added 2026-08-26) fertigation-pump electricity are all computed in kWh, but no €/kWh tariff, cost, or revenue figures exist yet.
- **Hydroponic Level A complete (2026-08-26)** (`hydroponics.md`) — system type corrected from the wrong "NFT" to "drip_substrate", and EC/pH/drainage-fraction targets now really drive irrigation water volume, drainage volume, fertigation pump electricity, and an approximate fertilizer mass (all derived from the crop's own transpiration, same reporting-layer pattern as the electricity fields above). **Level B, the "damped recipe" tier (N/K/Mg/B), and an informational-only tier (P/S/Fe/Mn/Zn/Cu/Mo) are fully designed but not started** — see `hydroponics.md`'s "Level B and beyond" section for the complete plan: real sourced numbers for EC→fruit-weight and EC→BER-risk (Level B, meant to be rigorously calibrated), sufficiency-range mechanisms for N/K/Mg/B (real mechanism, deliberately small/capped magnitude given higher uncertainty), and a reference-only concentration table for the remaining nutrients. Also logs a **Level D** idea (ML-based recalibration of the damped-tier coefficients once real greenhouse sensor/yield data exists) as a distant future item, not started or scaffolded. Implementation order (Level B alone first, vs. all tiers together) still needs deciding with the user.
- **Climate control is not yet phase-aware** — `climate_control` setpoints (heating, CO2, dehumidification) are flat constants for the whole `duration_days` run; real growers change climate strategy as the crop matures (e.g. different day/night temperature targets during vegetative vs. full-fruiting phases). The crop model already has phase boundaries (`crop.fruiting_start_days`, `crop.fruiting_ramp_days` — see the "Cross-check, 2026-08-26" section of `crop-model.md` for the reporting-only version of this, `summary.steady_state_truss_rate_per_week`), but nothing in `climate_control` reads them yet. **Planned next**, after hydroponics Level B/C above.
