...
- **Active dehumidification has no capacity limit** (`climate_control.dehumidification_setpoint_pct`, added 2026-08-20) — represents the real quote's OptiClima cooling/dehumidification panels, but modeled as always able to reach its RH setpoint; no real capacity spec was available. Replace with a real kg-water/hour capacity once the vendor's unit is known — see `climate-control.md`.
