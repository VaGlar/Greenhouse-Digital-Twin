...
## Real weather source added 2026-08-24 (`csv_typical_year`)

Added a third `weather.source` option, `"csv_typical_year"`, plus `scripts/fetch_weather.py`, to actually close the gap above for the real site — Αλεξάνδρεια Ημαθίας (40.63°N, 22.47°E), per the user. Source: [Open-Meteo Historical Weather API](https://open-meteo.com/en/docs/historical-weather-api) (free, no API key, CC BY 4.0 attribution — see the script's docstring for commercial-use licensing caveats), 10 years (2015–2024) of hourly `temperature_2m`, `relative_humidity_2m`, `shortwave_radiation`, averaged per (month, day, hour) into a repeatable "typical year" — see `twin/weather.py`'s module docstring for why a typical year rather than exact historical dates (simulations run against future planting dates real history can't cover directly).

**Not yet active** — this sandboxed dev session cannot reach `archive-api.open-meteo.com` (network egress is allowlisted to a small set of domains; both a direct request and the WebFetch tool were blocked with `EGRESS_BLOCKED`). The fetch script must be run once from an environment with normal internet access to produce `config/weather/alexandreia-imathias-typical-year.csv`; `config/greenhouse_example.yaml` still points at `source: synthetic` until that file exists and the config is switched over. Two ways to run it: manually trigger the **"Fetch weather data"** GitHub Actions workflow (`.github/workflows/fetch-weather.yml` — Actions tab → Run workflow; it runs on a GitHub-hosted runner with normal internet access and commits the CSV automatically, no local setup needed), or run `python scripts/fetch_weather.py` locally/in a Codespace terminal. Once switched, `latitude_deg`/`mean_annual_temp_c`/`seasonal_amplitude_c`/`diurnal_amplitude_c`/`peak_solar_w_m2` above stop mattering for this site (they're only inputs to the synthetic generator) — replaced by whatever the real 10-year average actually shows.
