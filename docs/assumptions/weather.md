...
**Not yet active** — this sandboxed dev session cannot reach `archive-api.open-meteo.com` (network egress is allowlisted to a small set of domains; both a direct request and the WebFetch tool were blocked with `EGRESS_BLOCKED`). The fetch script must be run once from an environment with normal internet access (e.g. inside the Codespace itself) to produce `config/weather/alexandreia-imathias-typical-year.csv`; `config/greenhouse_example.yaml` still points at `source: synthetic` until that file exists and the config is switched over. Once switched, `latitude_deg`/`mean_annual_temp_c`/`seasonal_amplitude_c`/`diurnal_amplitude_c`/`peak_solar_w_m2` above stop mattering for this site (they're only inputs to the synthetic generator) — replaced by whatever the real 10-year average actually shows.
