...
υγρασίας, ενεργειακής κατανάλωσης και ωρών κλειστής θερμοκουρτίνας.
...
- **Καμία λεπτομερής υδροπονία** — το σύστημα (`hydroponic.system_type: "NFT"`)
  είναι μόνο ένα flag, χωρίς μοντελοποίηση θρεπτικών/EC/pH/fertigation. Η ίδια
  η επιλογή "NFT" μάλλον δεν είναι σωστή για τομάτα (πραγματικά παραδείγματα
  όπως τα Θερμοκήπια Σαββίδη και η προσφορά της Γεωθερμική α.ε. δείχνουν
  σύστημα υγροπονίας σε υπόστρωμα με fertigation, όχι NFT) — επόμενο πεδίο
  δουλειάς.
- **Το κλίμα δεν αλλάζει ανά φάση καλλιέργειας** — τα setpoints θέρμανσης/CO2/
  αφύγρανσης είναι σταθερά σε όλη τη διάρκεια της προσομοίωσης, ενώ ένας
  πραγματικός καλλιεργητής αλλάζει στρατηγική καθώς ωριμάζει το φυτό
  (βλαστική ανάπτυξη vs. πλήρης καρποφορία).
...
- **Βαθύτερη μοντελοποίηση υδροπονίας** (σωστό system type — πιθανότατα
  υπόστρωμα/fertigation αντί για NFT, θρεπτικά, EC/pH, στόχος drainage %) —
  **επόμενο βήμα** (αποφασίστηκε 2026-08-26).
- **Phase-aware κλιματικό μοντέλο**: διαφορετικά setpoints θέρμανσης/CO2/
  αφύγρανσης ανά φάση καλλιέργειας (βλαστική / έναρξη καρποφορίας / πλήρης
  καρποφορία), αντί για σταθερά setpoints σε όλη τη διάρκεια — **σχεδιασμένο
  μετά** την υδροπονία (αποφασίστηκε 2026-08-26).
